...
export async function createProCheckoutSession(email: string): Promise<string> {
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://intelligence.sheisai.ai';
  ...
  const session = await s.checkout.sessions.create({
    ...
    success_url: `${appUrl}/journey/pro?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url: `${appUrl}/`,
    allow_promotion_codes: true,
    payment_intent_data: {
      metadata: { product: 'intelligence-layer-pro' },
    },
    metadata: { product: 'intelligence-layer-pro' },
  });
